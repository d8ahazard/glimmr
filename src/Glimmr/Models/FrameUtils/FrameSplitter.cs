...
namespace Glimmr.Models.FrameUtils;
...
public class FrameSplitter : IDisposable {
	...
	private CropCounter _pCropCounter;
	...
	public FrameSplitter(ColorService cs, bool crop = false) {
		...
		_pCropCounter = new CropCounter(_cropDelay);
	...
	private async Task CheckCrop(Mat image) {
		...
		if (_cropLetter) {
			...
			if (_lCropCounter.Triggered != _lCrop && !_sectorChanged) {
				_sectorChanged = true;
				_lCrop = _lCropCounter.Triggered;
			}
		...
		if (_cropPillar) {
			...
			_pCropCounter.Tick(pPixels);
			if (_pCropCounter.Triggered != _pCrop && !_sectorChanged) {
				_sectorChanged = true;
				_pCrop = _pCropCounter.Triggered;
			}

...
namespace HueDream.Models.Hue {
    public class HueBridge {
        ...
        public void UpdateLights(string[] colors, int brightness, CancellationToken ct, double fadeTime = 0) {
            if (entLayer != null) {
                ...
                foreach (var entLight in entLayer) {
                    ...
                    var xyColor = HueColorConverter.RgbToXY(endColor, GetLightGamut(lightData.ModelId));
                    endColor = HueColorConverter.XYToRgb(xyColor, CIE1931Gamut.PhilipsWideGamut);
        ...
        private CIE1931Gamut GetLightGamut(string modelId) {
            ...
            return Array.Exists(modelsC, element => element == modelId) ? CIE1931Gamut.ModelTypeC : CIE1931Gamut.PhilipsWideGamut;
        }
        ...
        public List<LightData> GetLights() {
            ...
            foreach (var light in ld) {
                var add = true;
                foreach (var unused in lights.Where(oLight => oLight.Id == light.Id)) {
                    add = false;
                }

...
namespace Glimmr.Models.CaptureSource.Video {
	public sealed class VideoStream {
		...
		public void StartCapture(bool hasDs, bool hasSd) {
			LogUtil.Write("Beginning video capture process...");
			SetCapVars();
			var autoEvent = new AutoResetEvent(false);
			saveTimer = new Timer(SaveFrame, autoEvent, 5000, 5000);
			LogUtil.WriteInc($"Starting vid capture task, setting sw and h to {_scaleWidth} and {_scaleHeight}");
			...
			while (!_cancellationToken.IsCancellationRequested) {
				...
				var sectors2 = StreamSplitter.GetSectorsV2();
				
				//LogUtil.Write("No, really, sending colors...");
				_colorService.SendColors(colors, sectors, sectors2);
			}
		...
		public void Refresh(bool hasDs, bool hasSd) {
			StreamSplitter = new Splitter(_ledData, _scaleWidth, _scaleHeight, hasDs, hasSd);
		}
